...
export default function RightSideBarFollower() {
  ...
  const followerId = useStore((state) => state.followerId);

  const filterFollowerId = followerInfo
    ? followerInfo.filter((e) => e.userId === followerId)
    : [];

  const [objectifiedArray] = filterFollowerId || [{}];

  console.log(objectifiedArray);

  return (
    <>
      {filterFollowerId && filterFollowerId.length > 0 && (
        <div>
